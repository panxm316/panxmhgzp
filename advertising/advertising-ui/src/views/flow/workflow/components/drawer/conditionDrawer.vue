...
import { TFlowCondition } from '@/types/views/flow/flowcondition'
...
const openEvent = () => {
  ...
  getFlowConditionListApi(groupId).then(({ obj }: IAxios<TFlowCondition[]>) => {
    console.log(obj)
    const dataList: any[] = []
    obj.forEach((item) => {
      const data = {
    ...
    if (i !== conditionsConfig.value.conditionNodes.length - 1) {
      ...
      for (var it of conditionList) {
        ...
        for (var ite of it.conditionList) {
          if (isBlank(ite.key) || isBlank(ite.expression) || isBlank(ite.value)) {
            error = true
            break
          }
        }
