 ...
 * 表格时间格式化
 */
export function formatDatesm(cellValue: string | number | Date) {
  if (cellValue == null || cellValue === '') return ''
  var date = new Date(cellValue)
  var year = date.getFullYear()
  var month = date.getMonth() + 1 < 10 ? '0' + (date.getMonth() + 1) : date.getMonth() + 1
...
export const computTreeHeighttab = () => {
  const treeheight = window.innerHeight - parseInt(variables.treeHeight) - 50
  return treeheight + 'px'
}
/**
 * 树结构右侧编辑区域高度
 ...
 * 数组从小到大排序
 */
export const sortNumber = (data: number[]) => {
 ...
 * 时间转换
 */
export const myGetTime = (time: any) => {
  const yy = time.getFullYear()
  const mm = time.getMonth() + 1
  const dd = time.getDate()
  const hh = time.getHours()
  const mf = time.getMinutes()
 ...
 * @description: 金额格式化
 */
export const formatMoney = (s: any, n: any) => {
