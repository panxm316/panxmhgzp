...
export const selectDictLabels = (datas: any, value: string | undefined, separator: undefined) => {
  ...
  var temp = value.split(currentSeparator) as any
 ...
 */
// prettier-ignore
export const maintitle = (title: string, language: string, lengths: any) => {
  let _length = 15
  if (language !== 'ukk') {
    if (lengths && lengths.length && lengths[0]) {
      _length = lengths[0]
    }
    return title.length > _length ? title.substr(0, _length) + '...' : title
  } else {
    if (lengths && lengths.length && lengths[1]) {
      _length = lengths[1]
    }
    return title.length > _length ? title.substr(0, _length) + '...' : title
  }
