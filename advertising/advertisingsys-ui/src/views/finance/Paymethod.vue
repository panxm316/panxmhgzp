 ...
 * @LastEditTime: 2023-10-24 11:04:47
 * @LastEditors: songly
 * @Description:付款方式
-->
<template>
  <div class="app-container">
    ...
    <el-dialog
      ...
      @close="closeDialog"
    >
      <el-form ref="ruleFormRef" :rules="rules" :model="paymethodForm" label-width="120px">
        <el-row :gutter="50">
          <el-col :span="22">
            <el-form-item label="名称" prop="sname">
              <el-input v-model="paymethodForm.sname" />
            </el-form-item>
            <el-form-item label="描述" prop="sdesc">
              <el-input v-model="paymethodForm.sdesc" />
            </el-form-item>
            <!-- <el-form-item label="备注" prop="sremark">
              <el-input v-model="paymethodForm.sremark" />
            </el-form-item> -->
            <el-form-item label="序号" prop="isort">
              <el-input-number
                v-model="paymethodForm.isort"
                :min="1"
                :max="99999"
            ...
            <el-form-item label="启用" prop="buse">
              <el-checkbox v-model="paymethodForm.buse" />
            </el-form-item>
            <el-form-item label="默认" prop="bdefault">
              <el-checkbox v-model="paymethodForm.bdefault" />
            </el-form-item>
...
import type { IPaymethod } from '@/types/views/finance/Paymethod'
import {
  getPaymethodPageListApi,
  savePaymethodApi,
  updatePaymethodApi,
  getPaymethodByIdApi,
  deletePaymethodApi,
  getMaxSortApi
} from '@/api/finance/Paymethod'
...
/** 校验设置参数 */
const rules = reactive<FormRules<IPaymethod>>({
  sname: [
    { validator: required, required: true, message: '请输入名称', trigger: 'change' },
    { max: 40, message: '不得大于40个字符', trigger: 'change' }
  ],
  sdesc: [
    { validator: required, required: true, message: '请输入描述', trigger: 'blur' },
    // { validator: isNumberStr, message: '请输入数字', trigger: 'change' },
    { max: 80, message: '不得大于40个字符', trigger: 'change' }
  ],
  sremark: [
    { validator: required, required: true, message: '请输入备注', trigger: 'change' },
    { max: 40, message: '不得大80个字符', trigger: 'change' }
  ]
})
/** 列表高度 */
const tableheight = ref(0)
/** 列表数据 */
const paymethodlist = ref<IPaymethod[]>()
/** 弹出添加修改标志 */
const dialogVisible = ref(false)
/** 表单对象 */
const paymethodForm = ref<IPaymethod>({
  id: undefined,
  sname: '',
  buse: true,
  isort: 1,
...
/** 列表多选数据 */
const multipleSelection = ref<IPaymethod[]>([])
...
const loaddata = () => {
  ...
  getPaymethodPageListApi(data).then(({ obj, total, success }: IAxios<IPaymethod[]>) => {
    if (success) {
      paymethodlist.value = obj
 ...
 * @param row
 */
const onEdit = (row: IPaymethod) => {
  dialogVisible.value = true
  nextTick(() => {
    paymethodForm.value = { ...row }
  })
}

const onDelete = (row?: IPaymethod) => {
  ...
  modal.confirm('是否删除选中数据？').then(() => {
    deletePaymethodApi(ids.join(',')).then(({ success }: IAxios) => {
      if (success) {
...
const onSubmitForm = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate((valid, fields) => {
    if (valid) {
      if (paymethodForm.value.id) {
        updatePaymethodApi(paymethodForm.value).then(({ success }: IAxios) => {
          if (success) {
        ...
        savePaymethodApi(paymethodForm.value).then(({ success }: IAxios) => {
          if (success) {
            modal.msgSuccess('保存成功')
            onResetForm(formEl)
            loaddata()
 ...
 * @param val
 */
const handleSelectionChange = (val: IPaymethod[]) => {
 ...
 */
const showhelp = () => {
  const to = '/template/Help?name=' + 'paymethod'
