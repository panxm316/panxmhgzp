  ...
  >
    <el-tabs type="border-card">
      <el-tab-pane label="设置审批人">
        ...
        <template v-if="approverConfig.refuse?.handler && approverConfig.assignedType === 11">
          <h4>审批被拒绝</h4>
...
import { isNode, isNotBlank, deepCopy } from '@/utils/index'
...
function produceSerialNodeList(parentId, process, nodeArr, nodeObj, noBranch) {
  ...
  if (!isNotBlank(parentId)) {
    ...
    const parentType = nodeObj[parentId].type
...
const isMultiUserForm = (id) => {
  ...
  const t = step2FormUserList.value.filter(res => res.id === id)[0].props.multi
  return t
}
