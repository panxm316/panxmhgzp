          ...
          if (groupCondition.conditionList) {
            for (const con of groupCondition.conditionList) {
              ...
              if (
                ...
                valueElement.type === 'SingleSelect' ||
                valueElement.type === 'MultiSelect'
              ) {
                valueShow = value?.map((res: any) => res.value).join(',')
              } else if (valueElement.type === 'ConditionRelation') {
                // valueShow = '广告条件'
                const list = conditionRelationStr(value as any)
                valueShow = list.join()
              ...
              } else if (expression === 'anyMatch') {
                conArr.push(name + ' 符合其一 ' + valueShow)
              } else if (expression === 'noneMatch') {
                conArr.push(name + ' 全不符合 ' + valueShow)
              } else {
                if (valueElement.type === 'ConditionRelation') {
                  conArr.push(name + valueShow)
                }
              }
            }
          }
...
const conditionRelationStr = (conditionList: any[]) => {
  const groupConArr = []
  const conArr: any[] = []
  if (conditionList) {
    for (const con of conditionList) {
      const { key, expression, value } = con
      if (!key) {
        continue
      }
