 ...
 * @LastEditTime: 2023-09-08 13:26:35
 * @LastEditors: wanghl
 * @Description: 部门树组件
-->
<template>
  ...
  <el-row v-if="props.filterable">
    <el-col :span="24">
      <el-input v-model="queryKey" placeholder="请输入部门" @change="searchZtree">
        <template #append>
  ...
  <el-row>
    <div :id="ztreeId" :style="'height: ' + ztreeHeight + 'px;overflow: auto;'" class="ztree"></div>
  </el-row>
</template>
...
/** ztreeId */
const ztreeId = ref(new Date().getTime() + 'treeid')
/** ztree请求url 根据相应需求url添加 */
let ztreeUrl = ''
switch (props.scopeflag) {
  case EHgDeptZtreeUrl.DEPT_GETSYSDEPTTREE:
    ztreeUrl = import.meta.env.VITE_APP_BASE_API + '/system/dept/getSysDeptTree'
    break
  case EHgDeptZtreeUrl.DEPT_GETSYSEMPDEPTTREE:
    ztreeUrl = import.meta.env.VITE_APP_BASE_API + '/system/dept/getSysEmpDeptTree'
    break
  ...
  view: {
    ...
    fontCss: function (treeid, treeNode) {
      return treeNode.buse
        ? { color: '#F56C6C', 'font-weight': '600', 'font-size': '16px!important' }
