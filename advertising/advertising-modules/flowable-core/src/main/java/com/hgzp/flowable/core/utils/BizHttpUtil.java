package com.hgzp.flowable.core.utils;
...
public class BizHttpUtil {
    ...
    public static String get(String url) {
        ...
        return HttpRequest.get(StrUtil.format("{}{}", bizUrl, url))
                .header(SecurityConstants.FROM_SOURCE, SecurityConstants.INNER)
                .addInterceptor(tLogHutoolhttpInterceptor).execute().body();
     ...
     * @param userId 用户id
     */
    public static R<List<DeptDto>> queryParentDepListByUserId(String userId) {
     ...
     * @param userId 用户id
     */
    public static R<DeptDto> queryDeptByUserId(String userId) {
        String s = get("/remote/queryDeptByUserId?userId=" + userId);
        R<DeptDto> r = JSON.parseObject(s, new TypeReference<R<DeptDto>>() {
        });
        return r;
    }
     ...
     * @param userIdList
     */
    public static List<String> replaceHolidayUser(List<String> userIdList) {
        String post = post(userIdList, "/remote/replaceHolidayUser");
        R<List<String>> listR = JSON.parseObject(post, new TypeReference<R<List<String>>>() {
        });
        return listR.getObj();
    }
