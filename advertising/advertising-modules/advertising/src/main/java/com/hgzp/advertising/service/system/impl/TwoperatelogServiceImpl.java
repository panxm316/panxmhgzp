...
import com.hgzp.mapper.system.TwoperatelogMapper;
 ...
 */
@Service
public class TwoperatelogServiceImpl extends ServiceImpl<TwoperatelogMapper, Twoperatelog> implements TwoperatelogServiceI {

    @Override
    public void saveAsyncOperatelogWithoutUser(Twoperatelog twoperatelog) throws Exception {
        LoginUser loginUser = WebUtil.getLoginUser();
        String clientIp = ServletUtil.getClientIP(WebUtil.getHttpServletRequest());

        twoperatelog.setEmployid(loginUser.getUserid());
        twoperatelog.setEmployname(loginUser.getUsername());
        twoperatelog.setSoperateip(clientIp);
        twoperatelog.setSoperatetime(new Date());

        saveAsyncOperatelog(twoperatelog);
    }


    @Async
    @Override
    public void saveAsyncOperatelog(Twoperatelog twoperatelog){
        this.save(twoperatelog);
    }

    @Override
    public Page<Twoperatelog> getOperatelogPageList(Page<Twoperatelog> page, BaseQueryInfo query, String slogtype) {
        LambdaQueryChainWrapper<Twoperatelog> twoperatelogLqw = this.lambdaQuery();
        twoperatelogLqw.ge(query.getStartTime() != null, Twoperatelog::getSoperatetime, query.getStartTime());
        if (query.getEndTime() != null) {
            twoperatelogLqw.lt(Twoperatelog::getSoperatetime, DateUtil.offsetDay(query.getEndTime(), 1));
        }
        Page<Twoperatelog> twoperatelogPage = twoperatelogLqw
                .eq(StrUtil.isNotBlank(slogtype), Twoperatelog::getSlogtype, slogtype)
                .and(StrUtil.isNotBlank(query.getQueryKey()), i -> i.like(Twoperatelog::getEmployname, query.getQueryKey())
                        .or()
                        .like(Twoperatelog::getSremark, query.getQueryKey())
                        .or()
                        .like(Twoperatelog::getSoldvalue, query.getQueryKey())
                        .or()
                        .like(Twoperatelog::getSnewvalue, query.getQueryKey()))
                .page(page);

        return twoperatelogPage;
    }
