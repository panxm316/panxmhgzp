...
import com.hgzp.advertising.service.system.impl.TwparameterServiceImpl;
...
public class MainController {
    ...
    @Autowired
    TwparameterServiceI parameterService;
    ...
    @PostMapping("/login")
    public Json login(String account, String password, String uuid, String code, HttpServletRequest request,
     ...
     * @author wangwk
     */
    private Json loginByHighSecurityLevel(String account, String password, HttpServletRequest request,
        ...
        if (!SecurityUtils.matchesPassword(password, userpassword)) {
            ...
            redisService.setCacheObject(CacheConstants.PWD_ERR_CNT_KEY + tbemploy.getSusername(), retryCount,
                    lockLoginTime, TimeUnit.MINUTES);
     ...
     */
    private Json<LoginInfoVo> loginCommon(String account, String password, HttpServletRequest request,
                                          HttpServletResponse response) throws Exception {
        LoginInfoVo loginInfoVo = new LoginInfoVo();
        Tbemploy tbemploy = tbemployService.getTbemployByLoginName(account);
        if (tbemploy == null) {
    ...
    public Json<LoginInfoVo> getInfo() throws Exception {
        ...
        List<String> permissions = tbroleService.getUserPermissions(u.getUserid());
        //peij 20230418 cassys地址
//        String cassysURL = baseSetParameterService.getParameterValue(ParameterEnum.CASSYS_URL);
        // peij 20231115 office预览地址
        String officePreviewUrl = parameterService.getParameterByKey("officePreviewUrl");
        loginInfoVo.setOfficePreviewUrl(officePreviewUrl);
    ...
    public Json<List<RouterVo>> getRouters() throws Exception {
        LoginUser u = WebUtil.getLoginUser();
        List<RouterVo> menus = tbmenuService.getMenuRouterByUserId(u.getUserid(), MenuClassConstants.ADVERTISING);
     ...
     *
     * @param
     * @return com.hgzp.core.page.Json<java.util.List < com.hgzp.core.page.Menu>>
     * @author songly
     * @date 2024/1/25 16:57
     */
    @RequestMapping("/getMyRouters")
    public Json<List<Menu>> getMyRouters() throws Exception {
        LoginUser u = WebUtil.getLoginUser();
        List<Menu> menus = tbmenuService.getMymenu(u.getUserid());
        return Json.success(menus);
    }
}
